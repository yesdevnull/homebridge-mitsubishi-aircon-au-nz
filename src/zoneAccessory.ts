...
import { Unit, Zones } from './data.js';
...
export class ZoneAccessory {
  ...
  async handleOnGet(): Promise<CharacteristicValue> {
    ...
    const zones: Zones[] = this.accessory.context.device.state.zones;
    //returns the stored zone information block based on name e.g. { zoneid: 1, status: 1, name: 'Dining' }
    const zoneinfo = zones.find(zone => zone.name === this.accessory.displayName);
