...
export class HeatCoolService extends AbstractService {
  ...
  async setActive(value: CharacteristicValue) {
    ...
    if (Object.prototype.hasOwnProperty.call(b, 'zones') && (b.zones.length || b.zones.length >= 2)) {
      ...
      for (let k = 0; k < b.zones.length; k++) {
        ...
        if (existingzoneaccessory) {
          const service = existingzoneaccessory.getService(this.hap.Service.Fanv2);
          if (service) {
          ...
          this.platform.log.error('No exisitingzoneaccessory found...Zone (name/ID):', zone.name, zone.zoneid, zone.displayName); 
        }
      } // zone end Loop
    //end zones defined. 
    } else {
